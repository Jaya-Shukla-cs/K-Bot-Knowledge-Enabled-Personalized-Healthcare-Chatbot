...
# This section of code to be run after scraping the data from Practo
...
class MainForm(Frame):
    ...
    def __init__(self,master=None):
        ...
        super().__init__(master=master)
        master.title("KBOT")
        master.config(bg="#008080")
        self.frame1 = Frame(master, bg="#14D2DC")
        self.frame1.pack(fill='y')
        self.frame2 = Frame(master)
        self.frame2.pack(side="left", padx=50, pady=2)
        self.frame3 = Frame(master)
        self.frame3.pack(side="right", padx=100, pady=60)
        self.createWidget()
    def createWidget(self):
#==========================================Heading and icon=========================================================
        self.img_load = Image.open("kboticon2.png")
        self.img_load = self.img_load. resize((100, 100), Image. ANTIALIAS)
        self.image_icon = ImageTk.PhotoImage(self.img_load)
        self.icon = Label(self.frame1,image=self.image_icon)
        self.icon.grid(row=0,column=0)
        self.Lb1 = Label(self.frame1, text="K-Bot", bg="#14D2DC", fg="white", font=('Georgia', 44), borderwidth ="4")
        self.Lb1.grid(row=0,column=1)
        self.Lb1 = Label(self.frame1, text="Knowledge Enabled Personalised Healthcare Chatbot", bg="#14D2DC", fg="white", font=('Georgia', 34), borderwidth ="4")
        self.Lb1.grid(row=0,column=2)
#================================================Image=======================================================
        self.img_load1 = Image.open("kbot_health1.png")
        self.img_load1 = self.img_load1. resize((800, 500), Image. ANTIALIAS)
        self.image_health = ImageTk.PhotoImage(self.img_load1)
        self.icon = Label(self.frame2, image=self.image_health, borderwidth = 0)
        self.icon.grid()

#==============================================Login=============================================================
        self.Lbl_user = Label(self.frame3,text = "Username :",bg="#14D2DC",fg="white",font=('Georgia',20))
        self.Lbl_user.grid(row=1,column=1,padx=10,pady=10)
        self.Lbl_pass = Label(self.frame3,text = "Password :",bg="#14D2DC",fg="white",font=('Georgia',20))
        self.Lbl_pass.grid(row=2,column=1,padx=10,pady=10)

        self.user_var = StringVar()
        self.pass_var = StringVar()
        self.Ent_user = Entry(self.frame3,textvariable = self.user_var,bg="#14D2DC",fg="white",font=('Georgia',15),relief="raised")
        self.Ent_user.grid(row=1,column=2)
        self.Ent_pass = Entry(self.frame3,textvariable = self.pass_var,show="*",bg="#14D2DC",fg="white",font=('Georgia',15),relief="raised")
        self.Ent_pass.grid(row=2,column=2)

        self.btn_login = Button(self.frame3, text = "Login", bg="#14D2DC", fg="white", font=('Georgia',20), command=self.btnLogin_Click)
        self.btn_login.grid(row=3,column=2)

        self.btn_register = Button(self.frame3,text = "Not Yet Registered?",bg="#14D2DC",fg="white",font=('Georgia',20),command=self.btnRegister_Click)
        self.btn_register.grid(row=4,column=2,pady=10)

    def btnLogin_Click(self):
            username1 = self.user_var.get()
            ...
            if username1 in list_of_files:
                ...
                if password1 in verify:
                    ...
                    frmQuestion = QuestionDigonosis(MainForm.main_Root)
                    frmQuestion.pack()
                else:
...
class SignUp(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.createWidget()
    def createWidget(self):
        self.lblMsg = Label(self, text="Please  enter ", bg="#14D2DC", fg="white", font=('Georgia', 34)) \
            .grid(row=0, column=0, pady=10)
        self.lblMsg=Label(self, text="your details ", bg="#14D2DC", fg="white", font=('Georgia', 34)) \
            .grid(row=0, column=1, pady=10)
        self.lblMsg1 = Label(self, text="below", bg="#14D2DC", fg="white", font=('Georgia', 34)) \
            .grid(row=0, column=2, pady=10)
        self.username_lable = Label(self, text="Username *  :", bg="#14D2DC", fg="white",
                                       font=('Georgia', 20))
        self.username_lable.grid(row=1, column=0, padx=10, pady=10)
        self.username = StringVar()
        self.username_entry = Entry(self, textvariable=self.username, font=('Georgia', 12), relief="raised")
        self.username_entry.grid(row=1, column=1, padx=10, pady=10)
        self.password_lable = Label(self, text="Password *  :", bg="#14D2DC", fg="white",
                                       font=('Georgia', 20))
        self.password_lable.grid(row=2, column=0, padx=10, pady=10)
        self.password = StringVar()
        self.password_entry = Entry(self, textvariable=self.password, show='*', font=('Georgia', 12),
                                       relief="raised")
        self.password_entry.grid(row=2, column=1, padx=10, pady=10)
        self.btnRegister=Button(self, text="REGISTER", width=10, height=1, bg="#14D2DC", fg="white",
                       font=('Georgia', 12), command=self.register_user) \
            .grid(row=3, column=1)
